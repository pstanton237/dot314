...
function scoreModelCandidate(
	...
	if (collapsedId.startsWith(collapsedQuery)) {
		return 6_000 - (collapsedId.length - collapsedQuery.length) + providerBonus;
	}
...
export async function resolveAuthenticatedModelCandidate(
	modelQuery: string,
	candidates: ScopedModelCandidate[],
	modelRegistry: { getApiKeyAndHeaders(model: any): Promise<RequestAuthResolution> },
	preferredProvider?: string,
): Promise<
	| { ok: true; candidate: ScopedModelCandidate; auth: SubagentRequestAuth }
	| { ok: false; error: string }
> {
	let firstAuthError: string | undefined;
	for (const candidate of rankModelCandidates(modelQuery, candidates, { preferredProvider })) {
		const auth = await modelRegistry.getApiKeyAndHeaders(candidate.model);
		const authError = getRequestAuthError(candidate.model, auth);
		if (auth.ok && !authError) return { ok: true, candidate, auth };
		firstAuthError ??= authError;
	}
	return {
		ok: false,
		error: firstAuthError ?? `No matching model with request auth found for "${modelQuery}".`,
	};
}
...
export default function (pi: ExtensionAPI) {
	...
	function renderBtwResult(r: SingleResult, theme: any): InstanceType<typeof Box> {
		...
		box.addChild(
			new Text(
				`${icon} ${theme.fg("toolTitle", theme.bold("btw: "))}${badge}` +
	...
	pi.registerCommand("btw", {
		...
		handler: async (args, ctx) => {
			...
			if (modelOpt) {
				...
				if (!matched.ok) {
					ctx.ui.notify(matched.error, "error");
					return;
				}

				targetModel = matched.candidate.model as any;
				requestAuth = matched.auth;
				if (!modeOpt && matched.candidate.thinkingLevel) {
					thinkingLevel = matched.candidate.thinkingLevel;
				}
			}
			...
			if (!ctx.modelRegistry.getProvider(targetModel.provider)) {
				ctx.ui.notify(`Provider "${targetModel.provider}" is unavailable.`, "error");
				return;
			}

			if (!requestAuth) {
				const auth: RequestAuthResolution = await ctx.modelRegistry.getApiKeyAndHeaders(targetModel);
				if (!auth.ok) {
			...
			const messages = sessionManager.buildSessionContext().messages.filter(
				(message) => !(
					message.role === "custom"
					&& "customType" in message
					&& message.customType === BTW_MESSAGE_TYPE
				),
			);
			...
			if (modelOpt) resolved.applied.model = modelOpt;
			const agentBadge = formatAgentBadge({ ...resolved.applied, unresolved: resolved.unresolved });
