...
Install dot314 and enable `pi-codex-goal` with `pi config`:

```sh
pi install git:github.com/w-winter/dot314
```

Avoid enabling another installation of `pi-codex-goal` in the same Pi configuration because duplicate copies register the same commands and tools.

## Changes from upstream

The baseline is [fitchmultz/pi-codex-goal](https://github.com/fitchmultz/pi-codex-goal) 0.1.36. This version schedules mid-run compaction at the next `context` event before a provider request after a qualifying turn that uses a tool, so Pi persists the complete tool call and result before compaction changes provider context. The smoke test that exercises Pi's SDK runtime uses its current `ModelRuntime` API.
