...
export function createTurnEventHandlers(deps: GoalRuntimeTurnHandlerContext) {
  ...
  const maybeScheduleProactiveCompaction = (
    message: TurnEndEvent["message"],
    ctx: ExtensionContext,
  ): void => {
    if (runtimeState.proactiveCompactionPhase !== "idle") {
      return;
    }
    ...
    // The next context event is the first provider boundary after the complete tool turn has settled.
    runtimeState.proactiveCompactionPhase = "scheduled";
  };
  ...
  return {
    ...
    onTurnEnd: (async (event, ctx) => {
      ...
      if (isAbortedAssistantMessage(event.message)) {
        if (runtimeState.proactiveCompactionPhase === "compacting") {
          return;
        }
        runtimeState.proactiveCompactionPhase = "idle";
      ...
      maybeScheduleProactiveCompaction(event.message, ctx);
