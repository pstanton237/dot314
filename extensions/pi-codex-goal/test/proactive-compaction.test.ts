...
test("tool-use turn_end schedules compaction at the next provider-context boundary", async () => {
...
test("a user abort before the provider-context boundary cancels scheduled compaction", async () => {
  const harness = createRuntimeHarness({ contextUsage: contextUsage(OVER_THRESHOLD) });
  await harness.runCommand("ship it");
